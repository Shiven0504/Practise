const DEFAULT_POWER_ITERATION_TOLERANCE = 1e-9;
...
function randomUnitVector(n) {
  const vector = Array.from({ length: n }, () => Math.random() - 0.5);
...
function powerIteration(M, maxIter = 1000, tol = DEFAULT_POWER_ITERATION_TOLERANCE) {
  const n = M.length;
  let b = normalize(randomUnitVector(n));
  let lambda = 0;
